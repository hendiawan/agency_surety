	        ...
	        columns: [
	            ...
	            { data: 'jenis_sppsb', name: 'jenis_sppsb',
	            	render: function ( data, type, row ) {
	            		var title = "";
	            		var fee = ""; 
	                    if(data=='1')title="Penawaran";
	                    else 
                                          if(data=='2')title="Pelaksanaan";
	                    else 
                                          if(data=='3')title="Uang Muka";
	                    else 
                                          if(data=='4')title="Pemeliharaan";
	                    else 
                                          if(data=='5')title="pembayaran";
	                    else 
                                          if(data=='6')title="Sanggah Banding";
	            ...
	            { data: 'status', name: 'status', sClass:'text-center',
	                render: function ( data, type, row ) {
	                    var label = "", title="",status_bayar=row.status_bayar;
                                           if (row.status_bayar=="Ya") {status_bayar="<b style='color:green'>Sudah</b>";} else {status_bayar="<b style='color:red'>Belum</b>";}
	                    if(data=='D'){ label="label-default"; title="Draf" }
	                    else if(data=='B'){ label="label-primary"; title="Baru"}
	                    else if(data=='I'){ label="label-info"; title="Direktur"}
                                             else if(data=='P'){ label="label-info"; title=="Direktur Utama"}
                                            ...
                                            else if(data=='H'){ label="label-danger"; title="Reject" }
                                            else if(data=='K'){ label="label-warning"; title="Kabag" }
                                            if (row.fee==null) fee="Tidak Ada";
                                            else
                                            {
                                                fee=row.fee+" %";
                                            }
	                    return '<span class="label '+label+' label-mini '+row.direksi+'">\n\
                                                       '+ title +'</span>\n\
                                                       <br>Bayar : '+status_bayar+
                                                       '<br>Fee : '+fee;
	                }
	            },
